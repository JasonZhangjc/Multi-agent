...
f2 = strcat('GG(TP([',num2str(setdiff(1:100,Pass)),'],[3]))');             %not too many robots in narrow passage way

% surveil two regions by leaving them once in a while
f3 = strcat('GF(TP([',num2str(Room1),'],[3]))');        % GF([0.2 labels, >=5])
f4 = strcat('GF(TP([',num2str(Room2),'],[3]))');        % GF([0.8 labels, >=5])        
f5 = strcat('GF(Neg(TP([',num2str(Room1),'],[1])))');   % GF([0.2 labels, <=0])
f6 = strcat('GF(Neg(TP([',num2str(Room2),'],[1])))');   % GF([0.8 labels, <=0])
f7 = strcat('FG(Neg(TP([',num2str(Room3),'],[1])))');   % FG([1 labels, <= 1]) at steady state at most one robot is left in the lower part

f8 = strcat('GF(TP([',num2str(setdiff(1:100,Room1)),'],[5]))');   % GF([0.2 labels, <=0])
f9 = strcat('GF(TP([',num2str(setdiff(1:100,Room2)),'],[5]))');   % GF([0.8 labels, <=0])
f10 = strcat('FG(TP([',num2str(setdiff(1:100,Room3)),'],[4]))'); 

%f7 = strcat('U(Neg([',num2str([pass_ends,1]),']),And([',num2str([72 74 1]),'],[',num2str([22 24 1]),']))');
f11 = strcat('TP(GFI([',num2str(CS),']),[5])');
f12 = strcat('TP(GFI([',pass,']),[5])');
...
fm1 = strcat('And(',f1,',',f2,',',f6,',',f7,',',f4,',',f9,',',f10,',',f11,')');
fm2 = strcat('And(',f3,',',f5,',',f8,',',f12,')');
...
n = size(A,1);
W0=zeros(n,1);
W0 = [ones(5,1); zeros(n-5,1)];
M0=zeros(n,1);
M0 = [zeros(5,1); ones(5,1); zeros(n-10,1)];
NW = sum(W0);
NM = sum(M0);
...
[W, Wtotal, Z, mytimes, sol] = main_template7(fm1,A,h,W0,Obs,CA_flag,M0,fm2);
...
save('data100.mat','W','W','Wtotal','Wtotal','ZLoop','ZLoop','A','A','mygrid','mygrid', 'Z', 'Z','sol','sol');


grid_plot_100('data100.mat');
% grid_plot(filename);
